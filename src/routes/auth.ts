...
export async function authRoutes(app: FastifyInstance) {
  app.post("/register", async (request) => {
    ...
    const accessTokenResponse = await axios.post("https://github.com/login/oauth/access_token", null, {
      params: { client_id: process.env.GITHUB_CLIENT_ID, client_secret: process.env.GITHUB_CLIENT_SECRET, code },
    ...
    if (!user) {
      user = await prisma.user.create({
        data: { name: userInfo.name, login: userInfo.login, githubId: userInfo.id, avatarUrl: userInfo.avatar_url }
      })
    }
